...
const YuZhouApp = lazyApp(() => import('../apps/YuZhouApp'));
...
const APP_IDLE_PRELOAD_ORDER: PreloadableLazy[] = [
  ...
  SpecialMomentsApp, CharCreatorDevApp, ForumApp,
];
...
const APP_BY_ID: Partial<Record<AppID, PreloadableLazy>> = {
  ...
  [AppID.Novel]: NovelApp, [AppID.Reading]: MingLightApp, [AppID.Moments]: MomentsApp, [AppID.Bank]: BankApp, [AppID.XhsStock]: XhsStockApp,
  ...
  [AppID.WorldHome]: WorldHomeApp, [AppID.YuZhou]: YuZhouApp,
  [AppID.Forum]: ForumApp,
};
...
const PhoneShell: React.FC = () => {
  ...
  const renderApp = () => {
    switch (activeApp) {
      ...
      case AppID.YuZhou: return <YuZhouApp />;
          ...
          <div className="absolute top-12 left-0 w-full flex flex-col items-center gap-2 pointer-events-none z-[60]">
              {toasts.map(toast => (
                 <div
                   ...
                   className={`animate-fade-in bg-white/95 backdrop-blur-xl px-4 py-3 rounded-2xl shadow-xl border border-black/5 flex items-start gap-3 max-w-[85%] ring-1 ring-white/20 ${toast.onClick ? 'pointer-events-auto cursor-pointer active:scale-95 transition-transform' : ''}`}
                 >
